...
# Vercel env — never committed. Falls back to a per-process random secret
# for local dev so `uvicorn` still runs with zero config; that means
# restarting the dev server invalidates existing sessions, which is fine
# for local dev but is exactly why production needs a real, persistent one.
...
# they're sent on cross-site requests (frontend on Vercel, backend on Fly —
# different domains). Locally, the Vite dev server proxies same-origin, so
# Secure=False / SameSite=Lax works without needing HTTPS in dev.
...
def _extract_access_token(request: Request) -> str | None:
    ...
    # the API can be exercised directly with curl/Postman for testing —
    # required by the acceptance criteria ("ยิงคำขอตรงๆ").
...
def get_current_user(request: Request, db: Session = Depends(get_master_db)) -> models.User:
    ...
    # remains service-owned — we map the verified human to the local user by
    # email and apply the LOCAL role (never the ecosystem role blindly).
    try:
...
# Convenience: anyone logged in except client_viewer — used to block the
# read-only role from every write endpoint and from internal-only routers
# (tasks/notes/activity/board-items/comments/whiteboards) in one shot.
require_internal = require_roles("pmo_admin", "dev", "qa")
# Sign-off (document confirm) authority — simplified from the spec's
# per-project role assignment (no per-project role table exists yet) to a
# global role check: pmo_admin and qa can sign off, dev and client_viewer
# cannot.
require_signoff_role = require_roles("pmo_admin", "qa")

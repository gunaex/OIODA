...
from ..import_schemas import BOARD_ITEMS as SCHEMA, BOARD_STATUSES_BY_TYPE
...
# From import_schemas — export emits importable + export-only columns, which
...
def update_board_item(
    ...
    # Board items were the one workflow module never writing to activity_log,
    # so nothing they did showed up in the reports (or in the Progress Matrix's
    # derived actual dates). Same shape as tasks/functions/documents.
    diffs = {key: (getattr(obj, key), value) for key, value in updates.items() if getattr(obj, key) != value}
    for key, value in updates.items():
        setattr(obj, key, value)
    if "severity" in updates:
        obj.sla_due_date = compute_sla_due_date(obj.severity, obj.created_at, master_db)
    if diffs:
        log_changes(db, "board_item", item_id, diffs, changed_by)
    db.commit()
    db.refresh(obj)
    return obj
...
def export_board_items(slug: str, type: str = Query(...), db: Session = Depends(get_project_db)):
    ...
    return export_response(rows, EXPORT_COLUMNS, f"{slug}-{type}.xlsx")


@router.get("/import-template")
def board_import_template(slug: str):
    return template_response(SCHEMA, "board-items-import-template.xlsx")


@router.post("/import")
async def import_board_items(
    slug: str,
    file: UploadFile = File(...),
    db: Session = Depends(get_project_db),
    master_db: Session = Depends(get_master_db),
):
    """Board items were export-only until now, so a spreadsheet could come out
    but never go back in. Status is validated per flavour, since the three
    share one column but not one lifecycle."""
    ...
    for index, record in enumerate(records):
        ...
        allowed = BOARD_STATUSES_BY_TYPE.get(item_type, ())
            ...
            errors.append(
                {
                    ...
                    "column": "status",
                    "value": status,
                    "problem": f"not valid for a '{item_type}' — allowed: {', '.join(allowed)}",
                }
            )
    if errors:
        raise HTTPException(
            status_code=400,
            detail={
                "message": f"{len(errors)} cell(s) could not be imported. Nothing was saved.",
                "errors": errors,
    ...
    for record in records:
        ...
        if not data.get("item_code"):
            data["item_code"] = generate_item_code(db, item_type)
        if not data.get("status"):
            data["status"] = DEFAULT_STATUS[item_type]
        obj = models.BoardItem(
            **data,
            sla_due_date=compute_sla_due_date(data.get("severity"), datetime.utcnow(), master_db),
        )

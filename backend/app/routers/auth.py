...
# ---------- User management (pmo_admin only) ----------
# Not in the spec's explicit endpoint list, but necessary supporting
# infrastructure: without this, only the bootstrap admin account could ever
# exist, and RBAC (e.g. client_viewer restrictions) would be untestable.
...
def create_user(
    ...
    db: Session = Depends(get_master_db),
    _admin: models.User = Depends(require_roles("pmo_admin")),
):
...
def list_users(db: Session = Depends(get_master_db), _admin: models.User = Depends(require_roles("pmo_admin"))):

...
def read_import_excel(file_bytes: bytes, expected_columns: list[str]) -> list[dict[str, Any]]:
    ...
    # non-nullable-as-NaN column type. Casting to object first avoids the
    # coercion, so None actually sticks.
...
def read_import_csv(file_bytes: bytes, expected_columns: list[str]) -> list[dict[str, Any]]:
    try:

"""
PM Again — Ecosystem auth boundary (PM-E6).
...
for tenant or entitlement decisions in ecosystem mode. Mirrors Conductor's
own app/orchestration/ecosystem_auth.py for consistency across the
ecosystem.
"""
...
def require_ecosystem_identity(
    ...
    """Resolves the caller's tenant. LEGACY_LOCAL_AUTH (app/auth.py) still
    supplies human session identity/role — migrating human session auth
    fully to Account Again IdentityClaims is out of PM-E6's scope (disclosed
    limitation). What IS enforced here, live, is Account Again's authority
    over TENANT and PRODUCT ENTITLEMENT in ecosystem mode: a valid local
    session and the right global role are not enough on their own
    (LOCAL_AUTH_NOT_AUTHORITATIVE_IN_ECOSYSTEM_MODE)."""
    ...
    decision = account_again_client.evaluate_entitlement(
        tenant_id=tenant_id, product_id=PM_AGAIN_PRODUCT_ID, service_system_id=PM_AGAIN_PRODUCT_ID
    )
    if decision.get("decision") != "ALLOW":
        raise HTTPException(
            status_code=403,
            detail={
                "error": "PM_AGAIN_ENTITLEMENT_DENIED",

"""QA Again's client to Account Again — identity/tenant/entitlement
authority. Mirrors the pattern used elsewhere in the AGAIN ecosystem
(Conductor's app/integration/account_again_client.py, PM Again's
app/ecosystem/account_again_client.py): synchronous httpx, fail-closed on
any transport/verification error (never implicit ALLOW), short-lived
caches rather than trusting a token forever.
"""
...
SYSTEM_ID = os.environ.get("ACCOUNT_AGAIN_SYSTEM_ID", "QA_AGAIN")
CLIENT_SECRET = os.environ.get("ACCOUNT_AGAIN_CLIENT_SECRET", "")
_JWKS_CACHE_TTL_SECONDS = 300
_ENTITLEMENT_CACHE_TTL_SECONDS = 30
_TOKEN_TIMEOUT_SECONDS = 5.0

# Account Again's local dev service-auth issuer/audience — see
# ACCOUNT-AGAIN/account_again/services/service_auth.py. Matches PM Again's
# and Conductor's own expectation exactly; this is not QA Again's own value
# to define.
...
class AccountAgainUnavailableError(Exception):
    """Raised when Account Again cannot be reached for an outbound call
    (e.g. obtaining QA Again's own service token). Callers must fail
    closed."""
...
# ── Inbound: verifying a caller's service token (e.g. CONDUCTOR_MAIN) ──
...
def _public_key_for(kid: Optional[str]):
...
def verify_service_token(token: str) -> dict:
    ...
    claims. Raises ServiceAuthError on any failure — expired, bad
    signature, wrong issuer/audience, or the JWKS endpoint itself being
    unreachable. This is the only place QA Again trusts a caller's claimed
    service identity — never an X-Service-Name-style header."""
    try:
...
# ── Outbound: QA Again's own service token, for calls it makes out ──
...
class _TokenCache:
    ...
    def get(self) -> str:
        with self._lock:
            if self._token and time.time() < self._expires_at - 15:
                return self._token
...
def evaluate_entitlement(*, tenant_id: str, product_id: str = "QA_AGAIN", service_system_id: str = "QA_AGAIN") -> dict:

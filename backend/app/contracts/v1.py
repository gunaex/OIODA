"""
Canonical AGAIN-ECOSYSTEM v1 contract bindings (QA-E1).
Mirrors contracts/vendored/v1/schemas/*.json exactly. Canonical authority is
AGAIN-ECOSYSTEM — see vendored/manifest.json.

QARequest and QAResult are QA Again's primary bindings (consumed from /
produced for Conductor Main, per contracts/v1/README.md). The remaining nine
are bound so QA Again can reference EngineeringResult / InfrastructureResult
evidence and validate the full v1 set, without QA Again owning their
semantics.
"""
...
class QARequest(CanonicalModel):
    """Consumed from Conductor Main (QA-E4). QA Again does not generate
    this — Conductor owns QARequest authority."""
...
class QAResult(CanonicalModel):
    """Produced by QA Again (QA-E3/QA-E5). QA Again is the authority for
    this contract; Conductor consumes it and must not recompute it."""
...
class DeliveryReadinessResult(CanonicalModel):
    """Reference-only. Conductor's final authority — QA Again never
    produces this."""

  ...
  return (
    <aside className="flex w-80 shrink-0 flex-col border-l border-line bg-surface-1">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3 text-[13px]">
        {!focus && <p className="text-slate-500">Select an object (requirement, table, field, artifact…) to anchor this panel.</p>}
        {focus && tab === "Comments" && (
          <div className="space-y-2">
            {annotations.length === 0 && <p className="text-slate-500">No annotations on {focus.label}.</p>}
            {annotations.map((a) => (
